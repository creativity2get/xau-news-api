...
def get_news():
    ...
    try:
        ...
        if page.status_code != 200:
            return jsonify({"error": f"Status {page.status_code} from Myfxbook"}), 500

        soup = BeautifulSoup(page.text, 'html.parser')
        rows = soup.select('tr.calendarRow')

        if not rows:
            return jsonify({"debug": "No calendarRow elements found. Site structure may have changed."}), 200
        ...
        if not news_list:
            return jsonify({"debug": "Rows found but no valid USD events parsed."}), 200
